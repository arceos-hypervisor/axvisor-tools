...
static void test_header_uses_little_endian_layout(void)
{
	...
	assert(ivc_app_encode_text(
		payload, sizeof(payload), IVC_APP_MESSAGE_REQUEST,
		0x0807060504030201ULL, (const uint8_t *)"abc", 3, &message_len));
...
static void test_encoders_reject_invalid_bounds(void)
{
	...
	assert(!ivc_app_encode_pattern(
		payload, IVC_APP_HEADER_LEN - 1, IVC_APP_MESSAGE_REQUEST, 1));
	assert(!ivc_app_encode_text(
		payload, sizeof(payload), IVC_APP_MESSAGE_ACK, 1, (const uint8_t *)"x",
		1, &message_len));
}
...
static void test_ack_body_preserves_peer_identity(void)
{
	...
	assert(!ivc_app_ack_body_is_valid(
		(const uint8_t *)"ack from subscriber", strlen("ack from subscriber")));
	assert(!ivc_app_ack_body_is_valid(
