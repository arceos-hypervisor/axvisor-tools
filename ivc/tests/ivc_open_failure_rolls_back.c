...
static void
reset_mocks(unsigned long create_request, unsigned long rollback_request)
{
...
int __wrap_ioctl(int fd, unsigned long request, ...)
{
	...
	if (request == expected_create_request)
	{
		...
		if (request == IVC_PUBLISH_CHANNEL)
		{
			...
			strncpy(
				publish_arg->device_name, "/dev/mock-publisher",
				sizeof(publish_arg->device_name) - 1);
		}
		else
		{
			ivc_subscribe_arg_p subscribe_arg = arg;
			strncpy(
				subscribe_arg->device_name, "/dev/mock-subscriber",
				sizeof(subscribe_arg->device_name) - 1);
		}
		return 0;
	}
...
static int expect_rollback(const char *operation)
{
	...
	{
		fprintf(
			stderr, "%s: expected one create ioctl, got %d\n", operation,
			create_requests);
		return 1;
	}
	if (open_requests != 1)
	{
		fprintf(
			stderr, "%s: expected one open call, got %d\n", operation,
			open_requests);
		return 1;
	}
	...
	{
		fprintf(
			stderr, "%s: expected one rollback ioctl, got %d\n", operation,
			rollback_requests);
		return 1;
	}
	...
	{
		fprintf(
			stderr, "%s: expected the channel device to open with O_RDWR\n",
			operation);
		return 1;
	}
	return 0;
