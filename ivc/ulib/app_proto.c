...
/* Exercise one-fragment and full-ring Message V1 boundaries. */
const size_t ivc_app_request_lengths[IVC_APP_REQUEST_COUNT] = {
	231, 232, 233, 7424, 7484,
};
...
{
	return kind == IVC_APP_MESSAGE_REQUEST || kind == IVC_APP_MESSAGE_ACK ||
...
static void ivc_app_encode_header(
	uint8_t *payload, enum ivc_app_message_kind kind, uint64_t sequence,
	uint16_t body_len)
{
...
static uint8_t ivc_app_pattern_byte(
	enum ivc_app_message_kind kind, uint64_t sequence, size_t index)
{
...
bool ivc_app_encode_pattern(
	uint8_t *payload, size_t payload_len, enum ivc_app_message_kind kind,
	uint64_t sequence)
{
	size_t body_len;
...
bool ivc_app_encode_text(
	uint8_t *payload, size_t payload_capacity, enum ivc_app_message_kind kind,
	uint64_t sequence, const uint8_t *body, size_t body_len,
	size_t *message_len)
{
	size_t required;
...
bool ivc_app_decode(
	const uint8_t *payload, size_t payload_len, struct ivc_app_message *message)
{
...
bool ivc_app_validate_pattern(
	...
	enum ivc_app_message_kind expected_kind, uint64_t expected_sequence,
	size_t expected_len)
{
