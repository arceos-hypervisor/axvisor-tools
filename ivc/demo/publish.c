...
#define POLL_INTERVAL_US 10000U
#define MAX_EMPTY_POLLS 3000U
...
static int send_requests(ivc_publisher_p publisher)
{
	...
	for (size_t index = 0; index < IVC_APP_REQUEST_COUNT; index++)
	{
		...
		if (!ivc_app_encode_pattern(
				payload, message_len, IVC_APP_MESSAGE_REQUEST, sequence))
		{
			fprintf(
				stderr,
		...
		if (ivc_publisher_send(publisher, payload, message_len) !=
			(int)message_len)
		{
			fprintf(
				stderr, "linux ivc send error: request seq=%llu len=%zu\n",
				(unsigned long long)sequence, message_len);
			return -1;
		}
		printf(
			"linux ivc send seq=%llu len=%zu\n", (unsigned long long)sequence,
			message_len);
	}
	return 0;
}
...
static int receive_replies(ivc_publisher_p publisher)
{
	...
	while (expected_ack_sequence <= IVC_APP_REQUEST_COUNT)
	{
		...
		int bytes_read =
			ivc_publisher_recv(publisher, payload, sizeof(payload));
		...
		case IVC_APP_MESSAGE_ACK:
				...
				!ivc_app_ack_body_is_valid(message.body, message.body_len))
			{
				fprintf(
					stderr,
					"linux ivc validation failed: ack expected=%llu "
					"actual=%llu len=%zu\n",
					(unsigned long long)expected_ack_sequence,
					(unsigned long long)message.sequence, message.body_len);
				return -1;
			}
				...
				expected_data_sequence != IVC_APP_DATA_COUNT + 1)
			{
				fprintf(
					stderr,
					"linux ivc validation failed: missing subscriber data "
					"expected=%llu\n",
					(unsigned long long)expected_data_sequence);
				return -1;
			}
			printf(
				"linux ivc ack seq=%llu msg=%.*s\n",
				(unsigned long long)message.sequence, (int)message.body_len,
		...
		case IVC_APP_MESSAGE_DATA:
		{
			...
			if (!ivc_app_validate_pattern(
					&message, IVC_APP_MESSAGE_DATA, expected_data_sequence,
					expected_len))
			{
				fprintf(
					stderr,
					"linux ivc validation failed: data expected=%llu "
					"actual=%llu len=%d\n",
					(unsigned long long)expected_data_sequence,
					(unsigned long long)message.sequence, bytes_read);
				return -1;
			}
			printf(
				"linux ivc recv data seq=%llu len=%d\n",
				(unsigned long long)message.sequence, bytes_read);
		...
		default:
			fprintf(
				stderr, "linux ivc validation failed: unexpected request on "
						"reply ring\n");
			return -1;
		}
	}
