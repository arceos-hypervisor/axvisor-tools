...
static int send_data_messages(ivc_subscriber_p subscriber)
{
	...
	for (size_t index = 0; index < IVC_APP_DATA_COUNT; index++)
	{
		...
		if (!ivc_app_encode_pattern(
				payload, message_len, IVC_APP_MESSAGE_DATA, sequence))
		{
			fprintf(
				stderr,
		...
		if (ivc_subscriber_send(subscriber, payload, message_len) !=
			(int)message_len)
		{
			fprintf(
				stderr, "linux ivc send error: data seq=%llu len=%zu\n",
				(unsigned long long)sequence, message_len);
			return -1;
		}
		printf(
			"linux ivc send data seq=%llu len=%zu\n",
			(unsigned long long)sequence, message_len);
	}
	return 0;
}
...
static int send_ack(ivc_subscriber_p subscriber, uint64_t sequence)
{
	...
	if (!ivc_app_encode_text(
			payload, sizeof(payload), IVC_APP_MESSAGE_ACK, sequence, ACK_BODY,
			sizeof(ACK_BODY) - 1, &message_len))
	{
		fprintf(
			stderr, "linux ivc validation failed: cannot encode ack seq=%llu\n",
			(unsigned long long)sequence);
		return -1;
	}
	if (ivc_subscriber_send(subscriber, payload, message_len) !=
	...
	{
		fprintf(
			stderr, "linux ivc send error: ack seq=%llu\n",
			(unsigned long long)sequence);
		return -1;
	}
...
static int receive_requests(ivc_subscriber_p subscriber)
{
	...
	while (received < IVC_APP_REQUEST_COUNT)
	{
		...
		int bytes_read =
			ivc_subscriber_recv(subscriber, payload, sizeof(payload));
		...
		if (!ivc_app_decode(payload, (size_t)bytes_read, &message) ||
			!ivc_app_validate_pattern(
				&message, IVC_APP_MESSAGE_REQUEST, expected_sequence,
				expected_len))
		{
			fprintf(
				stderr,
				"linux ivc validation failed: request expected=%llu len=%d\n",
				(unsigned long long)expected_sequence, bytes_read);
			return -1;
		}

		printf(
			"linux ivc recv request seq=%llu len=%d\n",
...
int main(int argc, char *argv[])
{
	...
	{
		fprintf(
			stderr, "Message V1 demo requires exactly %u requests\n",
